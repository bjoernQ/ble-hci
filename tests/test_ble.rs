...
fn init_fails() {
    ...
    assert_matches!(res, Err(ble_hci::Error::Failed));
...
fn create_le_set_advertising_parameters_works() {
    ...
    assert_eq!(
        data.data[..19],
        [0x01, 0x06, 0x20, 0x0f, 0x90, 1, 0x20, 3, 0, 0, 1, 0, 0, 0, 0, 0, 0, 1, 0]
    );
}
...
fn attribute_server_replies_to_group_type_requests() {
    ...
    assert_eq!(response_data.to_slice(), &[
        0x02, 0x00, 0x20, 0x0c, 0x00, 0x08, 0x00, 0x04, 0x00, 0x11, 0x06, 0x01, 0x00, 0x03, 0x00, 0x00, 0x28
    ]);

    // ReadByGroupTypeReq { start: 1, end: ffff, group_type: Uuid16(2800) }
    connector.reset();
    connector.provide_data_to_read(&[
        0x02, 0x00, 0x20, 0x0b, 0x00, 0x07, 0x00, 0x04, 0x00, 0x10, 0x03, 0x00, 0xff, 0xff, 0x00, 0x28
    ]);
    assert_matches!(srv.do_work(), Ok(()));
    ...
    connector.reset();
    connector.provide_data_to_read(&[
        0x02, 0x00, 0x20, 0x0b, 0x00, 0x07, 0x00, 0x04, 0x00, 0x08, 0x01, 0x00, 0x02, 0x00, 0x02, 0x28
    ]);
    assert_matches!(srv.do_work(), Ok(()));
    // check response (not found)
    let response_data = connector.get_written_data();
    ...
    connector.reset();
    connector.provide_data_to_read(&[
        0x02, 0x00, 0x20, 0x0b, 0x00, 0x07, 0x00, 0x04, 0x00, 0x08, 0x01, 0x00, 0x03, 0x00, 0x03, 0x28
    ]);
    assert_matches!(srv.do_work(), Ok(()));
    // check response (not found)
    let response_data = connector.get_written_data();
    assert_eq!(response_data.to_slice(), &[
        0x02, 
        0x00, 0x20, 0x1b, 0x00,
        0x17, 0x0, 0x4, 0x0, 0x9, 0x15, 0x2, 0x0, 0xa, 0x3, 0x0, 
        0xa8, 0x6a, 0x62, 0xf1, 0x5d, 0x26, 0x45, 0x38, 0xb3, 0x64, 0x56, 0x54, 0x96, 0x15, 0x15, 0xc9,
    ]);
    ...
    assert_eq!(response_data.to_slice(), &[
        0x02,
        0x00, 0x20, 0x0a, 0x00,
        0x06, 0x00, 0x04, 0x00, 0x0b, 0x48, 0x65, 0x6c, 0x6c, 0x6f,
    ]);
...
// TODO test with more than 1 service
